...
##     -c, --context <name>     Optimize ALL the icons in icons/src/<variant>/<context> and save them under icons/Sucharu/<context>
...
optimize() {
  ...
  OUTDIR=../../Sucharu${SUFFIX}/scalable/${GROUP}/
  OUTPUT=../../Sucharu${SUFFIX}/scalable/${GROUP}/${NAME}
  if [[ ! -d ${OUTDIR} ]]; then
    mkdir -p ${OUTDIR}
  fi
